...
namespace BinaryDataDecoders.PackMan.Cli
{
    public class NugetManager : INugetManager
    {
        ...
        {
            if (string.IsNullOrWhiteSpace(inputVersions)) yield break;

            foreach (var targetVersion in inputVersions.Split(';'))
            {
                var parts = targetVersion.Split(new[] { ',' }, 2);
        ...
        public void UpdateTargetVersions(XDocument xDocument, IEnumerable<(string name, string? version)> properties)
        {
            ...
            foreach (var property in properties)
            {
                ...
                {
                    _logger.LogInformation($@"Set ""{property.name}"" to ""{property.version}""");
        ...
        public async Task UpdateNugetVersionsAsync(XDocument versionXml, string nugetUrl, FeedType feedType, CancellationToken cancellationToken)
        {
            ...
            foreach (var package in packages)
            {
                ...
                if (lockedVersion)
                {
                    _logger.LogInformation($"Skip version check {packageName}: {version} => SKIPPED");
                    continue;
                }
                ...
                if (string.IsNullOrEmpty(condition))
                {
                    ...
                    if (target != null && target != current)
                    {
                        ...
                        _logger.LogInformation($"Found upgrade version {packageName}: {version} => {target}");
                        if (target.IsLegacyVersion)
                            _logger.LogInformation($"{packageName}: {target} is legacy, update soon");
                    }
                    else if (current.IsLegacyVersion)
                    {
                        _logger.LogInformation($"Already up to date {packageName}: {version}");
                        _logger.LogInformation($"{packageName}: {version} is legacy, update soon");
                    }
                }
                else
                {
        ...
        public void Save(XDocument versionXml, string filePath)
        {
            ...
            _logger.LogInformation($"Saved content to {filePath}");
        }

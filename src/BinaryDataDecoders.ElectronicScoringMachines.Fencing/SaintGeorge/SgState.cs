    ...
    public class SgState
    {
        ...
        public static IScoreMachineState Create(IScoreMachineState last, ReadOnlySpan<byte> frame)
        {
            ...
            if (frame.StartsWith(Dc3, S, T, Sotx))
            {
                ...
                return new ScoreMachineState(red, green, last?.Clock ?? TimeSpan.Zero);
            }
            ...
            else if (frame.StartsWith(Dc3, R, _, F, _S, Sotx))
            {
                ...
                return new ScoreMachineState(last?.Red ?? new Fencer(), last?.Green ?? new Fencer(), time);
            }
                     ...
                     )
            {
                ...
                return new ScoreMachineState(red, green, last?.Clock ?? new TimeSpan());
            }
